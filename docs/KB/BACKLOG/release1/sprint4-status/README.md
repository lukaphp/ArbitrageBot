# Meccanismo di stato autonomo — Sprint 4

Ogni membro del team scrive **solo il proprio file** in questa cartella. Nessun agente legge o
scrive il file di un altro (eccetto Roger, che li legge tutti per l'aggregato — mai per scriverli).
Questo evita qualunque race condition: non essendoci un file condiviso scritto da più processi, non
serve lock né coordinamento tra agenti. Stesso meccanismo degli Sprint 2-3.

## File

| File | Scritto da | Contenuto |
|:---|:---|:---|
| `sprint4-tasks.json` | Io/PO (statico, non da agenti) | Elenco canonico dei task, proprietario, story point — fonte: `sprint4.md`. Aggiornato dopo ogni planning. |
| `bruno.json` | bruno | Stato dei task che possiede: ADV-01, ADV-03 (+ parte backend di ADV-02 e ANA-01, item 1-3 di DEBT-01) |
| `joshua.json` | joshua | Stato dei task che possiede: OBS-01 (+ modulo tasso di CUR-01, item 4-5 di DEBT-01) |
| `maya.json` | maya | Stato dei task UI: drawer di ADV-02, UI di CUR-01 e ANA-01 |
| `annie.json` | annie | Note di review (ADV-01, ADV-02, ADV-03, DEBT-01) + candidati di refinement |
| `aggregate.json` | roger | Sintesi di tutti i file sopra, letta da me per aggiornare la board visuale |

**Task con doppio proprietario (ADV-02, CUR-01, ANA-01, DEBT-01):** ciascun proprietario aggiorna
la propria parte nel proprio file — non esiste un file condiviso per un task a due, per non
reintrodurre la race condition che questo meccanismo evita. Roger concilia le metà in
`aggregate.json`.

OPS-02 e OPS-03r non hanno un file di stato dedicato: sono esplicitamente non delegati a un agente
(accesso VPS reale fuori dal loro perimetro) — restano in carico al PO, eseguiti insieme a Claude
nel thread principale.

## Schema di un file di stato agente

```json
{
  "agent": "bruno",
  "updatedAt": "2026-08-10T10:00:00Z",
  "tasks": {
    "ADV-01": {
      "status": "not_started | in_progress | ready_for_review",
      "criteriaChecked": [false, false, false, false],
      "notes": "testo libero: cosa resta, blocchi, decisioni prese"
    }
  }
}
```

**Regole:**
- `status` può arrivare fino a `ready_for_review`, **mai** a `done`/`fatto` — quel passaggio è
  una decisione esplicita del PO, fatta in review.
- `criteriaChecked` è un array di booleani nello stesso ordine dei criteri di accettazione elencati
  in `sprint4.md` per quel task — aggiornalo mano a mano, non solo a fine lavoro.
- `notes` è dove segnali blocchi, decisioni non ovvie, o candidati per il refinement dello sprint
  successivo.
- Aggiorna il file **quando cambia qualcosa di reale** (non ad ogni singolo tool-call) — inizio
  lavoro, milestone interna, fine lavoro.

## Invarianti di questo sprint (da sprint4.md §1)

- Nessuna riga modificata in `riskAgent.js` o `proposals.approve()` per il lavoro advisor.
- La suite di prompt avversari (ADV-02) non si taglia se lo sprint va lungo.
- CUR-01 non tocca la logica di rischio: solo presentazione.

## Come arriva alla board visuale

Io leggo `aggregate.json` (prodotto da Roger) e aggiorno l'artifact pubblicato, come già per gli
Sprint 1-3.
