...
class PaperBroker {
    ...
    for (const [coin, pos] of [...acc.positions]) {
      ...
      for (const t of trigs) {
        const hit = pos.side === 'long'
          ...
          : (t.tpsl === 'sl' ? mid >= t.triggerPx : mid <= t.triggerPx);
        if (hit) {
          this._fillClose(master, coin, t.triggerPx, `trigger ${t.tpsl}`);
          break;
        }
      }
    }
  ...
  _fillClose(master, coin, px, reason) {
    const acc = this._acc(master);
    ...
    acc.fills.push({ time: Date.now(), coin, dir: `Close ${pos.side === 'long' ? 'Long' : 'Short'}`, px, sz: pos.size, fee, closedPnl });
  ...
  async placeMarketOrder({ masterAddress, coin, isBuy, size, slippage = DEFAULT_SLIPPAGE, reduceOnly = false }, network) {
    ...
    if (reduceOnly || (existing && existing.side === (isBuy ? 'short' : 'long'))) {
      ...
      this._fillClose(masterAddress, coin, px, 'market reduce');
      ...
      acc.equity -= fee;
      acc.fills.push({ time: Date.now(), coin, dir: `Open ${existing.side === 'long' ? 'Long' : 'Short'}`, px, sz: size, fee, closedPnl: 0 });
    } else {
      // Apertura nuova
      ...
      const fee = px * size * TAKER_FEE_PCT;
      acc.equity -= fee;
      acc.fills.push({ time: Date.now(), coin, dir: `Open ${side === 'long' ? 'Long' : 'Short'}`, px, sz: size, fee, closedPnl: 0 });
    }
  ...
  async closePosition({ masterAddress, coin }, network) {
    ...
    this._fillClose(masterAddress, coin, px, 'closePosition');
    return { oid: acc.oidSeq++, avgPx: px, error: null, paper: true };
  }
